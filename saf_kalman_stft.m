function out = saf_kalman_stft(mic, spk, frame_size, use_nlp)
    ...
    function st = init(frame_size)
        ...
        st.win_global = sqrt(hann(st.win_len+2));
        st.win_global = st.win_global(2:st.win_len+1)';
        % subband para
    ...
    function [st, out] = saf_process(st, mic_frame, spk_frame, use_nlp)
        ...
        ana_wined_echo = st.win_global .* st.ana_win_echo;
        ...
        ana_wined_far = st.win_global .* st.ana_win_far;
        ...
        comp_out = fft_out .* st.win_global;
